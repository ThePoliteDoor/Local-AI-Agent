from functions.get_files_info import get_files_info, schema_get_files_info
from functions.get_file_content import get_file_content, schema_get_file_content
from functions.write_file import write_file, schema_write_file
from functions.run_python_file import run_python_file, schema_run_python_file


# Schemas (for Ollama)
available_functions = [
    {"type": "function", "function": schema_get_files_info},
    {"type": "function", "function": schema_get_file_content},
    {"type": "function", "function": schema_write_file},
    {"type": "function", "function": schema_run_python_file},
]


# Real Python functions (for execution)
function_map = {
    "get_files_info": get_files_info,
    "get_file_content": get_file_content,
    "write_file": write_file,
    "run_python_file": run_python_file,
}


def call_function(function_call, working_directory, verbose=False):
    function_name = function_call.get("function", {}).get("name", "") or ""
    
    args = function_call.get("function", {}).get("arguments", {}) or {}
    if "args" in args and isinstance(args["args"], str):
        if args["args"].strip() == "[]":
            args["args"] = []

    if verbose:
        print(f"Calling function: {function_name}({args})")
    else:
        print(f" - Calling function: {function_name}")

    if function_name not in function_map:
        return {
            "role": "tool",
            "tool_name": function_name,
            "content": {
                "error": f"Unknown function: {function_name}"
            },
        }

    # Safe copy + sandbox
    safe_args = dict(args)
    safe_args["working_directory"] = working_directory

    # Call real function
    function_result = function_map[function_name](**safe_args)

    return {
        "role": "tool",
        "tool_name": function_name,
        "content": {
            "result": function_result
        },
    }
